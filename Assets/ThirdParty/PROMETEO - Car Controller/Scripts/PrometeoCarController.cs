...
public class PrometeoCarController : MonoBehaviour
{
    ...
    public int
        reversMultiplier = 2;
    [Space(10)] [Range(10, 90)]
    public int maxSteeringAngle = 27; // The maximum angle that the tires can reach while rotating the steering wheel.

    [Range(0.1f, 10f)] public float steeringSpeed = 0.5f; // How fast the steering wheel turns.
    ...
    */
    ...
    private void Update()
    {
        ...
        carSpeed = 2 * Mathf.PI * rearLeftCollider.radius * rearLeftCollider.rpm * 60 / 1000;
    ...
    public void ChangeControlls(bool forward)
    {
        _forward = forward;
    }
    public void Stop()
    {
        CancelInvoke("GoReverse");
        CancelInvoke("GoForward");
    ...
    public void MoveForward()
    {
        ...
        if (_forward)
        {
            ...
            InvokeRepeating("GoForward", 0f, 0.1f);
        ...
        else
        {
            ...
            InvokeRepeating("GoReverse", 0f, 0.1f);
    ...
    public float lerpFloat = 1;
    public void Turn(float angle)
    {
        steeringAxis = angle * steeringSpeed;
        steeringAxis = Mathf.Clamp(steeringAxis,-1, 1);
        var steeringAngle = steeringAxis * maxSteeringAngle;
        frontLeftCollider.steerAngle = Mathf.Lerp(frontLeftCollider.steerAngle, steeringAngle, steeringSpeed*Time.deltaTime*lerpFloat);
        frontRightCollider.steerAngle = Mathf.Lerp(frontRightCollider.steerAngle, steeringAngle, steeringSpeed*Time.deltaTime*lerpFloat);
    }
    ...
    public void GoForward()
    {
        ...
        else
        {
            if (Mathf.RoundToInt(carSpeed) < maxSpeed)
            {
                ...
                for (int i = 0; i < _trailerWheels.Count; i++)
                {
                    _trailerWheels[i].brakeTorque = 0;

                    _trailerWheels[i].motorTorque = accelerationMultiplier  * throttleAxis;
            ...
            else
            {
                ...
                for (int i = 0; i < _trailerWheels.Count; i++)
                {
                    _trailerWheels[i].motorTorque = 0;
    ...
    public void GoReverse()
    {
        ...
        else
        {
            if (Mathf.Abs(Mathf.RoundToInt(carSpeed)) < maxReverseSpeed)
            {
                ...
                frontLeftCollider.brakeTorque = 0;
                frontLeftCollider.motorTorque = reversMultiplier * 50f * throttleAxis;
                frontRightCollider.brakeTorque = 0;
                frontRightCollider.motorTorque = reversMultiplier * 50f * throttleAxis;
                rearLeftCollider.brakeTorque = 0;
                rearLeftCollider.motorTorque = reversMultiplier * 50f * throttleAxis;
                rearRightCollider.brakeTorque = 0;
                rearRightCollider.motorTorque = reversMultiplier * 50f * throttleAxis;
                for (int i = 0; i < _trailerWheels.Count; i++)
                {
                    _trailerWheels[i].brakeTorque = 0;

                    _trailerWheels[i].motorTorque = reversMultiplier * 50f * throttleAxis;
    ...
    public void Brakes()
    {
        ...
        for (int i = 0; i < _trailerWheels.Count; i++)
        {
            _trailerWheels[i].brakeTorque = brakeForce;
    ...
    public void Handbrake()
    {
        ...
        var secureStartingPoint = driftingAxis * FLWextremumSlip;

        if (secureStartingPoint < FLWextremumSlip) driftingAxis = FLWextremumSlip / FLWextremumSlip;
    ...
    private List<WheelCollider> _trailerWheels = new List<WheelCollider>();

    public void AddWheels(List<WheelCollider> pickZoneWheelColliders)
    {
        _trailerWheels = pickZoneWheelColliders;
        //accelerationMultiplier =(int)(accelerationMultiplier*0.5f) ;
        //reversMultiplier =(int)(reversMultiplier*0.75f) ;
        //maxSpeed =(int)(maxSpeed*0.75f) ; 
        
    }

    private bool _inGasStation = false;
    public void BlockMovement(bool inGas = false)
    {
        _inGasStation = inGas;
        _blockMovement = true;
        Stop();
